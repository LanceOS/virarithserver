...
<main class="mx-auto flex max-w-7xl flex-col gap-8 p-3 pb-12 sm:p-8">
    ...
    {:else if post}
        <article class="card-setup flex flex-col gap-8 p-6">
            <header class="relative flex items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex flex-col">
                        <button class="btn-nav font-semibold sm:text-lg" onclick={() => goto(`/pages/profile/${post?.user.id}`)}>{post.user.name}</button>
                        <time class="text-xs font-light sm:text-sm" datetime={formatDate(post.createdAt)} style="color: var(--color-muted);">
                ...
                <div class="flex gap-2 items-center"> <span class="text-sm sm:text-lg" style="color: var(--color-text-secondary);">{post.category.toUpperCase()}</span>
                    {#if hasActions}
                        <button class="stat-item actions-button" onclick={() => (openPostActions = !openPostActions)} aria-label="Post actions">
            ...
            <div class="mb-1 flex flex-col gap-2">
                ...
                <p class="space-y-4 text-sm sm:text-base break-words" style="color: var(--color-base-content);">
                    {@html post.content}
                </p>
            </div>
    ...
    {:else}
        <section class="card-setup flex min-h-64 flex-col items-center justify-center gap-4"> <Icon icon="material-symbols:error-outline" class="text-3xl" style="color: var(--color-error);" />
            <p class="text-center" style="color: var(--color-error);">Post not found or could not be loaded</p>
            <button
                class="btn-primary btn-medium" onclick={() => window.location.reload()}
            >
                Try Again
            </button>
        </section>
